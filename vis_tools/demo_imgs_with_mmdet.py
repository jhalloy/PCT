...
def vis_pose_result(image_name, pose_results, thickness, out_file):
    ...
    plt.savefig(out_file + '.jpg', format='jpg', bbox_inches='tight')
    plt.close()
...
def main():
    ...
    frames_wrist_locations = []

    # go through all images in args.img_root
    for image_name in sorted(os.listdir(args.img_root)):
        if not image_name.lower().endswith('.jpg'):
            print(f"expected jpg image, got {image_name}. Skipping.")
            continue

        image_name = os.path.join(args.img_root, image_name)
        print(image_name)
        ...
        dt_joints = np.array(pose_results[0]['keypoints']).reshape(17,-1)
        left_wrist = dt_joints[9][:2]
        right_wrist = dt_joints[10][:2]
        print(f"left wrist: {left_wrist}")
        print(f"right wrist: {right_wrist}")

        wrist_locations = []
        wrist_locations.extend(left_wrist)
        wrist_locations.extend(right_wrist)
        frames_wrist_locations.append(wrist_locations)

        if args.out_img_root == '':
            out_file = None
        else:
            os.makedirs(args.out_img_root, exist_ok=True)
            out_file = os.path.join(args.out_img_root, f'vis_{os.path.basename(image_name)[:-4]}')
    ...
    np_frames_wrist_locations = np.array(frames_wrist_locations)
    np.save('np_frames_wrist_locations.npy', np_frames_wrist_locations)
